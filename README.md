...
```
heather_telegram_bot.py         # Main bot entry point (Telethon userbot + Flask monitor)
heather_discord_bot.py          # Optional Discord bridge
persona_example.yaml            # Starter persona — copy to heather_personality.yaml
heather_kink_personas.yaml      # 17 adaptive kink persona overlays

heather/                        # Modular pipeline package
  config.py                     # Central config + tunables
  handlers.py                   # Incoming message routing
  conversation.py               # Conversation state + flow
  personality.py                # Persona loading + prompt assembly
  access_tiers.py               # FREE / FAN / VIP gating
  safety.py                     # CSAM hard-block, content guards
  humanize.py                   # Typing cadence, multi-bubble, delays
  postprocess.py                # Legacy response filter shim
  memory_vectors.py             # Semantic recall (embeddings + sqlite)
  persistence.py                # Per-user JSON profiles, tip/warmth state
  media_images.py               # Image library + delivery
  media_video.py                # Video delivery
  media_generation.py           # ComfyUI / FLUX on-demand generation
  image_analysis.py             # Incoming-photo vision (Grok / local fallback)
  voice.py                      # ElevenLabs + Coqui TTS
  monitoring.py                 # Web dashboard (port 8888)
  text_pipeline/                # Text generation pipeline
    pipeline.py                 # Orchestration: prompt -> LLM -> filter
    prompt_builder.py           # System + context prompt assembly
    llm_client.py               # llama-server (OpenAI-compatible) client
    response_filter.py          # Multi-stage output scrubbers

user_memory.py                  # Per-user memory + kink scoring
user_profiles/                  # Per-user JSON profiles (auto-generated, gitignored)
```
