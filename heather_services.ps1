...
$Config = @{
    ...
    # Active model — reverted 2026-06-15 to Cydonia-24B-v4.3.
    # The 2026-05-01 "Heretic" swap pointed at C:\Models\Qwen3.6-27B-Heretic\ but that
    # folder only contains a CODE model (NEO-CODE-HERE), which produced off-character chat.
    # The real Heretic chat GGUF was never present on disk. Re-download before re-trying it.
    LlamaServerModel = "C:\Models\Cydonia-24B-v4.3\Cydonia-24B-v4.3-Q6_K.gguf"
    LlamaServerGPULayers = 99
    LlamaServerContext = 16384  # Qwen3.6 27B Q5_K_M needs tighter context to fit single 3090 with KV
    # Qwen3.6 has thinking mode by default — disable for chat use (--reasoning-budget 0)
    LlamaServerReasoningBudget = "0"
    # --jinja: use the model's bundled chat template (Qwen ChatML / Cydonia Mistral)
...
function Start-LlamaServer {
    ...
    if ($Config.LlamaServerJinja) {
        $llamaArgs += "--jinja"
    }
    if ($null -ne $Config.LlamaServerReasoningBudget) {
        $llamaArgs += @("--reasoning-budget", $Config.LlamaServerReasoningBudget)
    }
...
function Start-AllServices {
    ...
    Start-Ollama
    # ComfyUI excluded from startall — only runs during nightly image generation
    # Start manually with: .\heather_services.ps1 -Action start -Service comfyui
    Start-TTS
    Start-TextBackend
...
function Start-AllServicesAuto {
    ...
    # ComfyUI excluded from autostart — only runs during nightly image generation
    Write-AutoLog "Skipping ComfyUI (on-demand only, started by nightly_images.ps1)"
