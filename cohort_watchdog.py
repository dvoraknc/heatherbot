...
# Paths
# ---------------------------------------------------------------------------
BOT_DIR = Path(r"C:\path\to\heatherbot")
LOG_PATHS = [
...
# ---------------------------------------------------------------------------
# Reply line: 2026-03-20 19:34:43 | INFO     | [R31729-2079] Reply to 1234567890 (1.9s): Oh that sounds...
RE_REPLY = re.compile(
    r"^(\d{4}-\d{2}-\d{2})\s+\d{2}:\d{2}:\d{2}\s+\|\s+INFO\s+\|\s+"
    r"(?:\[R\d+-\d+\]\s+)?Reply to (\d+)\s+\([^)]+\):\s+(.+)"
)

# User message: 2026-03-20 19:35:39 | INFO     | [R39275-2080] Text from Example User (1234567890) (chat): ...
RE_USER_MSG = re.compile(
    r"^(\d{4}-\d{2}-\d{2})\s+\d{2}:\d{2}:\d{2}\s+\|\s+INFO\s+\|\s+"
    r"(?:\[R\d+-\d+\]\s+)?Text from .+?\((\d+)\)\s+\(chat\):"
)

# IMAGE_LIB sent: [IMAGE_LIB] Sent nsfw_topless_058 (nsfw_topless) to 1234567890
RE_IMAGE_LIB = re.compile(
    r"\[IMAGE_LIB\]\s+Sent\s+\S+\s+\(\S+\)\s+to\s+(\d+)"
)

# Proactive selfie / library photo: Proactive selfie for 1234567890 / Proactive library photo sent to 1234567890
RE_PROACTIVE_IMG = re.compile(
    r"Proactive (?:selfie|library photo)\s+(?:for|sent to)\s+(\d+)"
)

# Voice note: Sent voice note to @example_user (1234567890)
RE_VOICE_NOTE = re.compile(
    r"Sent voice note to .+?\((\d+)\)"
)

# Voice welcome: [WELCOME] Sent voice welcome to @example_user (1234567890)
RE_VOICE_WELCOME = re.compile(
    r"\[WELCOME\]\s+Sent voice welcome to .+?\((\d+)\)"
)

# Video sent: Video sent to 1234567890: vid_061.mp4
RE_VIDEO_SENT = re.compile(
    r"Video sent to (\d+):"
)

# Sending cached video: Sending cached video vid_061.mp4 to 1234567890
