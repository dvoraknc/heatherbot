...
$botDir = 'C:\path\to\heatherbot'
$maxKillWaitSec = 15
$maxStartWaitSec = 60
$monitorPort = 8888
# Pinned python — must match the env where bot deps (yaml/telethon/flask) live.
# Bare `python` resolves differently when conda is active in the calling shell,
# which has caused silent startup deaths.
...
Start-Process -FilePath $python -ArgumentList 'heather_telegram_bot.py','--monitoring','--unfiltered','--log-dir','C:\AI\logs','--personality','heather_personality.yaml','--small-model' -WorkingDirectory $botDir -WindowStyle Hidden
