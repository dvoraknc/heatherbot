...
def strip_thinking_tags(text: str) -> str:
    """Remove <think>...</think> tags, Gemma4 <|channel>thought...<channel|> blocks, and unclosed variants."""
...
def strip_bracketed_metadata(text: str) -> str:
    ...
    # Session/memory summary leaks: [Session Summary: ...], [User Profile: ...], etc.
    text = re.sub(r'\[(?:Session Summary|Chat Summary|Conversation Summary|User Profile|Memory Summary|Relationship Summary|Session Notes|Context Notes|User Summary|Current State|Emotional State)[^\]]*\]', '', text, flags=re.IGNORECASE)
...
def strip_obvious_phantom_claims(text: str) -> str:
    ...
    obvious_patterns = [
        ...
        # "There we go" / "there you go" as a stand-alone implied-delivery line.
        # Keep anchored so metaphorical uses mid-sentence aren't stripped.
        r'^\s*there (?:you |we )(?:go|are)[\s.!?]*$',
        # "Now you've got something (to jerk to / a pic / a photo)" — phantom delivery.
        r"(?:now )?you'?(?:ve|\s+have)\s+got\s+(?:something|one|a\s+(?:pic|photo|selfie))",
        # "*snaps photos*" / "*takes a selfie*" — roleplay actions claiming to produce a photo.
        r"\*?\s*(?:snap|snapping|snapped|take|taking|took|click|clicking|clicked)s?\s+[^*\n]{0,40}?(?:pic|photo|selfie|shot)",
        # "*sends a selfie: ...*" / "*sending a photo*" — present-tense phantom send actions.
        r"\*?\s*(?:send|sends|sending|sent|attach|attaches|attaching|attached)s?\s+[^*\n]{0,40}?(?:pic|photo|selfie|video|vid|clip|shot|image)",
        # "*lifts/pulls shirt/top/bra*" — roleplay actions claiming visual display.
        r"\*?\s*(?:lift|lifts|lifting|lifted|pull|pulls|pulling|pulled|slip|slips|slipping|slipped)s?\s+(?:up|down|off)?\s*[^*\n]{0,20}?(?:shirt|top|bra|robe|dress|panties|clothes)",
    ]
...
AI_SELF_ID_PATTERNS = [
    ...
    re.compile(r"\bi exist (?:only |purely )?(?:as|in) (?:the )?digital\b", re.IGNORECASE),
    # Physical body denial patterns
    re.compile(r"\bi don'?t have a? ?(?:physical )?(?:body|form)\b", re.IGNORECASE),
    re.compile(r"\bi have no (?:physical )?(?:body|form)\b", re.IGNORECASE),
    re.compile(r"\bi don'?t (?:physically )?exist\b", re.IGNORECASE),
    re.compile(r"\bi'?m not a? ?physical\b", re.IGNORECASE),
    re.compile(r"\bno physical (?:presence|form|body)\b", re.IGNORECASE),
    # Assistant-mode self-identification
    re.compile(r"\bi'?m (?:designed|programmed|built|created) to\b", re.IGNORECASE),
    re.compile(r"\bmy (?:capabilities|features|purpose) (?:include|is)\b", re.IGNORECASE),
    re.compile(r"\bhere(?:'s| is) what i can do\b", re.IGNORECASE),
    re.compile(r"\bi'?m (?:capable of|able to help with)\b", re.IGNORECASE),
]
